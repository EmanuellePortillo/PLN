var Stemmer = {
    ...
    getNextVowelPos:function(word,start){
        ...
        for (var index = start; index < len; index++) {
            if(this.is_vowel(word.slice(index,1))){
                return index;
            }
        }
        return len;
    ...
    stemm: function(word){
        ...
        for (var index = 0; index < len-1 && r1 == len; index++) {
            if(this.is_vowel(word.slice(index,1)) && !this.is_vowel(word.slice(index+1,1))){
                r1 = index + 2;
            }            
        }
        ...
        if(len > 3){
            if(this.is_vowel(word.slice(1,1))){
                // If the second letter is a consonant, RV is the region after the next following vowel
                rv = this.getNextVowelPos(word,2) + 1;
            }else if(this.is_vowel(word.slice(0,1)) && this.is_vowel(word.slice(1,1))){
            ...
            var r1_txt = word.slice(r1);
            var r2_txt = word.slice(r2);
            var rv_txt = word.slice(rv);
            var word_orig = word;
            ...
            if(suf != ''){
                var pre_suff = this.endsinArr(rv_txt.slice(0,-suf.length),pronoun_suf_pre1);
                if (pre_suff != ''){
                    word = this.removeAccent(word.slice(0,-suf.length));
                }else{
                    pre_suff = this.endsinArr(rv_txt.slice(0,-suf.length),pronoun_suf_pre2);
                    if(pre_suff != '' || (this.endsin(word,'yendo') && word.slice(-suf.length-6,1) == 'u')){
            ...
            if(word != word_orig){
                r1_txt = word.slice(r1);
                r2_txt = word.slice(r2);
                rv_txt = word.slice(rv);
            }
            word_after0 = word;
            ...
            if(word_after0 == word_after1){
                ...
                if((suf = this.endsinArr(rv_txt,['ya', 'ye', 'yan', 'yen', 'yeron', 'yendo', 'yo', 'yó', 'yas', 'yes', 'yais', 'yamos'])) != '' && word.slice(-suf.length-1,1) == 'u'){
                    word = word.slice(0,-suf.length);
                }
...
function calculateStemm(data,c,lines){
    words = data.palabras;    
    var new_words = [];
    ...
    self.postMessage({'words': new_words,'i':c,'lines':lines});
}

//wait for the start 'CalculatePi' message
//e is the event and e.data contains the JSON object
self.onmessage = function(e) {
  calculateStemm(e.data.value,e.data.value.contador,e.data.value.lines);
}